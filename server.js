...
const database = new DatabaseSync(join(root, "manager.db"));
database.exec(`
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (relativePath.startsWith("..") || relativePath.split(/[/\\]/).some((part) => part.startsWith(".")) || ["server.js", "manager.db", "package.json"].includes(relativePath)) return json(response, 404, { error: "Not found" });
